...
require_once '../../app/controllers/KategoriAreaController.php';
...
$controller = new KategoriAreaController($pdo);
$kategoriAreas = $controller->index();
...
<body class="hold-transition sidebar-mini">
        ...
        <div class="content-wrapper">
            ...
            <section class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1>Data Kategori Area</h1>
                        </div>
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="#">Master Data</a></li>
                                <li class="breadcrumb-item active">Data Kategori Area</li>
                            </ol>
                        </div>
                    </div>
                </div><!-- /.container-fluid -->
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header row">
                                    <h3 class="card-title col-9">Berikut adalah list dari Data Kategori Area</h3>
                                    <button class="col-3 btn btn-block btn-success" data-toggle="modal"
                                        data-target="#modalTambah">
                                        Tambah Kategori Area
                                    </button>
                                </div>
                                <!-- /.card-header -->
                                <div class="card-body">
                                    <table id="example1" class="table table-bordered table-striped">
                                        <thead>
                                            <tr>
                                                <th>Kode Kategori Area</th>
                                                <th>Nama Kategori Area</th>
                                                <th>Deskripsi</th>
                                                <th>Urutan</th>
                                                <th>Status Aktif</th>
                                                <th>Aksi</th>
                                        ...
                                        <tbody>
                                            <?php foreach ($kategoriAreas as $kategoriArea): ?>
                                                <tr>
                                                    <td><?= htmlspecialchars($kategoriArea['kode']) ?></td>
                                                    <td><?= htmlspecialchars($kategoriArea['nama']) ?></td>
                                                    <td><?= htmlspecialchars($kategoriArea['deskripsi']) ?></td>
                                                    <td><?= htmlspecialchars($kategoriArea['urutan']) ?></td>
                                                    <td><?= $kategoriArea['is_active'] ? 'Aktif' : 'Nonaktif' ?></td>
                                                    <td class="row">
                                                        <div class="col">
                                                            <button class="btn btn-block btn-info btn-edit"
                                                                data-id="<?= $kategoriArea['id'] ?>"
                                                                data-kode="<?= htmlspecialchars($kategoriArea['kode']) ?>"
                                                                data-nama="<?= htmlspecialchars($kategoriArea['nama']) ?>"
                                                                data-deskripsi="<?= htmlspecialchars($kategoriArea['deskripsi']) ?>"
                                                                data-urutan="<?= $kategoriArea['urutan'] ?>"
                                                                data-status="<?= $kategoriArea['is_active'] ?>"
                        ...
                        <form id="formTambah" method="POST" action="store.php">
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    <label for="kode">Kode Kategori Area<code>*</code></label>
                                    <input type="text" name="kode" class="form-control" id="kode"
                                        value="<?= (new KategoriArea($pdo))->generateKode() ?>"
                                        placeholder="Masukkan kode kategori area">
                                </div>
                                <div class="form-group">
                                    <label for="nama">Nama Kategori Area<code>*</code></label>
                                    <input type="text" name="nama" class="form-control" id="nama"
                                        placeholder="Masukkan nama kategori area">
                                </div>
                                <div class="form-group">
            ...
            <div class="modal fade" id="modalEdit">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-header">
                            <h4 class="modal-title">Edit Kategori Area</h4>
                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                        </div>
        ...
        $(function () {
            function initValidation(formId) {
                $(formId).validate({
                    ...
                    messages: {
                        kode: {
                            required: "Silahkan masukkan Kode Kategori Area"
                        },
                        nama: {
                            required: "Silahkan masukkan Nama Kategori Area"
                        },
                        urutan: {
                            required: "Silahkan masukkan Urutan"
                        },
                        is_active: {
                            required: "Silahkan pilih Status Kategori Area"
                        }
                    },
                    errorElement: 'span',
    ...
    <script>
        ...
        if (success === "created") {
            showToast("Data Kategori Area berhasil ditambahkan", "created");
        }
        if (success === "updated") {
            showToast("Data Kategori Area berhasil diperbarui", "updated");
        }
        if (success === "deleted") {
            showToast("Data Kategori Area berhasil dihapus", "deleted");
        }
    </script>
</body>
