...
require_once '../../app/controllers/KondisiJalanController.php';
...
$controller = new KondisiJalanController($pdo);
$kondisiJalans = $controller->index();
...
<body class="hold-transition sidebar-mini">
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                ...
                                <li class="breadcrumb-item active">Data Kondisi Jalan</li>
                            </ol>
                        </div>
                    </div>
                </div><!-- /.container-fluid -->
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header row">
                                    <h3 class="card-title col-9">Berikut adalah list dari Data Kondisi Jalan</h3>
                                    <button class="col-3 btn btn-block btn-success" data-toggle="modal"
                                        data-target="#modalTambah">
                                        Tambah Kondisi Jalan
                                    </button>
                                </div>
                                <!-- /.card-header -->
                                <div class="card-body">
                                    <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            <?php foreach ($kondisiJalans as $kondisiJalan): ?>
                                                <tr>
                                                    <td><?= htmlspecialchars($kondisiJalan['kode']) ?></td>
                                                    <td><?= htmlspecialchars($kondisiJalan['nama']) ?></td>
                                                    <td><?= htmlspecialchars($kondisiJalan['deskripsi']) ?></td>
                                                    <td><?= htmlspecialchars($kondisiJalan['urutan']) ?></td>
                                                    <td><?= $kondisiJalan['is_active'] ? 'Aktif' : 'Nonaktif' ?></td>
                                                    <td class="row">
                                                        <div class="col">
                                                            <button class="btn btn-block btn-info btn-edit"
                                                                data-id="<?= $kondisiJalan['id'] ?>"
                                                                data-kode="<?= htmlspecialchars($kondisiJalan['kode']) ?>"
                                                                data-nama="<?= htmlspecialchars($kondisiJalan['nama']) ?>"
                                                                data-deskripsi="<?= htmlspecialchars($kondisiJalan['deskripsi']) ?>"
                                                                data-urutan="<?= $kondisiJalan['urutan'] ?>"
                                                                data-status="<?= $kondisiJalan['is_active'] ?>"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4 class="modal-title">Tambah Kondisi Jalan</h4>
                        ...
                        <form id="formTambah" method="POST" action="store.php">
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    <label for="kode">Kode Kondisi Jalan<code>*</code></label>
                                    <input type="text" name="kode" class="form-control" id="kode"
                                        value="<?= (new KondisiJalan($pdo))->generateKode() ?>"
                                        placeholder="Masukkan kode kondisi jalan">
                                </div>
                                <div class="form-group">
                                    <label for="nama">Nama Kondisi Jalan<code>*</code></label>
                                    <input type="text" name="nama" class="form-control" id="nama"
                                        placeholder="Masukkan nama kondisi jalan">
                                </div>
                                <div class="form-group">
            ...
            <div class="modal fade" id="modalEdit">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-header">
                            <h4 class="modal-title">Edit Kondisi Jalan</h4>
                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                        </div>
        ...
        $(function () {
            function initValidation(formId) {
                $(formId).validate({
                    ...
                    messages: {
                        kode: {
                            required: "Silahkan masukkan Kode Kondisi Jalan"
                        },
                        nama: {
                            required: "Silahkan masukkan Nama Kondisi Jalan"
                        },
                        urutan: {
                            required: "Silahkan masukkan Urutan"
                        },
                        is_active: {
                            required: "Silahkan pilih Status Kondisi Jalan"
                        }
                    },
                    errorElement: 'span',
    ...
    <script>
        ...
        if (success === "created") {
            showToast("Data Kondisi Jalan berhasil ditambahkan", "created");
        }
        if (success === "updated") {
            showToast("Data Kondisi Jalan berhasil diperbarui", "updated");
        }
        if (success === "deleted") {
            showToast("Data Kondisi Jalan berhasil dihapus", "deleted");
        }
    </script>
</body>
